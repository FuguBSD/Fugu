...
URL placeholders `{os}` and `{arch}` become the platform words. A `bin` line
that names a `.tar.gz`, a `.tgz` or a `.zip` URL takes a third field: the file
path in the archive. The runtime tier of this repository is empty by contract.
