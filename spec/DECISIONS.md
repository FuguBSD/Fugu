...
| D-06 | The perl floor is v5.34, the perl that macOS ships in base. A file that a Tooling pack owns keeps the floor of that pack. The `dist.perl` key of `.toolingrc` carries the floor into the distribution. | A consumer packs the modules into one file, and that file runs on a fresh macOS.          |
