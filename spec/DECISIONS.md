...
| D-06 | The Perl floor is v5.34, the perl that macOS ships in base. Every module, every test, and every Fugu-owned script must compile on it. | A program that packs Fugu into one file runs before any install, and a fresh macOS holds the system perl only. |
