...
starts nothing. These are the errors: the value is not a hash
reference; a name holds no character; a name holds an equals sign or
a NUL byte; a value is not defined; a value holds a NUL byte.
...
process continues after C<SIGKILL>.
